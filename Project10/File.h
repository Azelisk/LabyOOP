...
int del_from_file(const string& path, const int& count)
{
	...
	Pair number, p;
	cout << "Enter a pair to remove elements less than this:\n"; cin >> number;	//ââåäèòå ïàðó äëÿ óäàëåíèÿ ýëåìåíòîâ, ìåíüøå ÷åì ýòà
...
int add_elements(const string& path, const int& count)
{
	...
	cout << "After which element to add new (with " << 0 << " - to " << count << "): "; cin >> number;	//ïîñëå êàêîãî ýëåìåíòà äîáàâèòü íîâûå

	if (number < 0 or number > count)
		throw 3;

	cout << "Number of elements: "; cin >> new_count;	//êîëè÷åñòâî ââîäèìûõ ýëåìåíòîâ
	if (new_count < 1)
		throw 2;
